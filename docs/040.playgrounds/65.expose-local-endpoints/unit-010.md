...
```sh
labctl expose local PLAYGROUND_ID <local_addr>:<local_port> [--remote-port <port>]
```
...
```sh
labctl expose local $PLAY_ID 127.0.0.1:3000 --remote-port 8080
```

If your local service uses HTTPS (including self-signed certificates), add the `--https` flag:

```sh
labctl expose local $PLAY_ID 127.0.0.1:3000 --https
```
...
```sh
labctl expose local $PLAY_ID 127.0.0.1:3000 --public
```

To open the generated URL in your browser automatically, add `--open`:

```sh
labctl expose local $PLAY_ID 127.0.0.1:3000 --open
```
...
3. In another terminal, expose the local server as a public URL:

```sh
labctl expose local $PLAY_ID 127.0.0.1:4000 --public --open
```
